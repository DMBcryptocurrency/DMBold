...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <digitalmoneybitsaddress> [minconf=1]\n"
            "Returns the total amount received by <digitalmoneybitsaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <todigitalmoneybitsaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("digitalmoneybits-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("digitalmoneybits-lock-wa");
